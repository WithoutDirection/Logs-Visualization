...
class LogVisualizationApp {
    ...
    initializeGraphSelector() {
        ...
        graphIds.forEach(graphId => {
            ...
            option.textContent = `${metadata.name} (${metadata.stats.nodes} nodes, ${metadata.stats.edges} edges)`;
    ...
    onGraphSelectorChange(event) {
        ...
        if (selectedGraph) {
            const metadata = this.graphLoader.getGraphMetadata(selectedGraph);
            if (metadata) {
    ...
    updateStatsDisplay(metadata) {
        DOMHelper.setTextById('stat-nodes', metadata.stats.nodes.toLocaleString());
        DOMHelper.setTextById('stat-edges', metadata.stats.edges.toLocaleString());
        DOMHelper.setTextById('stat-entries', metadata.stats.entry_range[1].toLocaleString());
        ...
        const hasReapr = metadata.available_features.reapr_analysis;
